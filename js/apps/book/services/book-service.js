...
function getBook(searchParam) {
	const url = `https://www.googleapis.com/books/v1/volumes?printType=books&q=${searchParam}`;
	return axios.get(url).then((res) => {
		...
		const books = options.map((option) => {
			return {
				...
				subtitle: option.volumeInfo.subtitle,
				authors: option.volumeInfo.authors,
				publishedDate: option.volumeInfo.publishedDate.substring(0, 4),
				description: option.volumeInfo.description,
				pageCount: option.volumeInfo.pageCount,
				categories: option.volumeInfo.categories,
				thumbnail: option.volumeInfo.imageLinks.thumbnail,
				language: option.volumeInfo.language,
				listPrice: {
					amount: 0,
					currencyCode: 'ILS',
					isOnSale: false,
